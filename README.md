...
Yıldız Rover Destek Ekip - Ödev 2 kapsamında hazırlanmış, OpenCV ile renk tabanlı
"STOP" trafik işareti tespiti yapan bir Python uygulaması.
...
- Kırmızı renk HSV'de iki ayrı aralıkta bulunduğundan (0–10 ve 150–180), iki maske
  oluşturulup birleştirilir.
- Küçük kernel ile `MORPH_OPEN` uygulanarak gürültü temizlenir.
- Büyük kernel ile `MORPH_CLOSE` uygulanarak, tabela üzerindeki beyaz yazının
  kırmızı alanı bölmesinden kaynaklanan boşluklar kapatılır.
...
Algoritma yalnızca renge dayandığı için düşük ışık koşullarında ve tabela üzerindeki
yazının kırmızı alanı bölmesi durumunda kısmi/hatalı tespit yapabilmektedir. Detaylı
değerlendirme rapor dosyasında (`Odev2_Rapor.pdf`) bulunmaktadır.